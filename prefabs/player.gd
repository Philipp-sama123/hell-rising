...
func _physics_process(delta: float) -> void:
	...
	if shooting:
		_change_state(STATE_SHOOT); return
	if pre_aiming: return
	if not is_dashing and not is_hit: _select_state_from_motion()
...
func _handle_input(delta: float) -> void:
	...
	if jump_buffer_timer > 0 and not is_dashing and jumps_left == MAX_JUMPS and (is_on_floor() or coyote_timer > 0):
		jumps_left -= 1; jump_buffer_timer = 0; coyote_timer = 0; is_jumping = true; _change_state(STATE_JUMP); _apply_jump_velocity()
...
func _select_state_from_motion() -> void:
	...
	if not is_on_floor(): _change_state(STATE_JUMP if velocity.y < 0 else STATE_FALL)
	else:
		if abs_vx == 0: _change_state(STATE_IDLE)
		elif abs_vx > WALK_SPEED: _change_state(STATE_RUN)
		else: _change_state(STATE_WALK)
...
func _play_anim_with_aim(base_name: String) -> void:
	...
	if animated_sprite and animated_sprite.sprite_frames:
		if aiming:
			...
			if animated_sprite.sprite_frames.has_animation(aim): animated_sprite.play(aim); return
			if base_name == "Idle" and animated_sprite.sprite_frames.has_animation("Aim"): animated_sprite.play("Aim"); return
		if animated_sprite.sprite_frames.has_animation(base_name): animated_sprite.play(base_name); return
...
func _change_state(new_state: int) -> void:
	...
	match state:
		...
		STATE_AIM: _play_anim_with_aim(_get_motion_name_from_velocity())
		STATE_SHOOT:
...
func _get_motion_name_from_velocity() -> String:
	var abs_vx = abs(velocity.x)
	if is_on_floor():
		if abs_vx == 0: return "Idle"
		if abs_vx > WALK_SPEED: return "Run"
		return "Walk"
...
func _spawn_bullet() -> void:
	...
	if not b.is_in_group("bullet"): b.add_to_group("bullet")
	if b.has_method("set_shooter_owner"): b.set_shooter_owner(self)
	var dir_vec := Vector2(facing_dir, 0)
	if b.has_method("set_direction"): b.set_direction(dir_vec)
	elif b.has_method("set_velocity"):
		if b.has_variable("speed"): b.set_velocity(dir_vec * b.speed) 
		else: b.set_velocity(dir_vec * BULLET_SPEED)
